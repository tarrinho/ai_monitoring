...
### Added
- **Maintenance windows for backend DOWN/recovery alerts.** A known, expected outage (e.g. a daily vLLM model-reload restart) no longer pages: `MONITOR_MAINTENANCE_<BACKEND>` (one per `LITELLM`/`OLLAMA`/`LLAMACPP`/`VLLM`/`GPU`) takes a comma-separated list of `HH:MM-HH:MM` (UTC, 24h, daily-recurring, may cross midnight) windows during which that backend's DOWN/recovery alert is suppressed entirely — the dashboard still shows it down (collector data is untouched, only the alert path is silenced), and the anti-flap hysteresis streaks reset at the window boundary so a real outage past the window's end still pages after the normal `ALERT_BACKEND_DOWN_AFTER` consecutive-failures delay, not instantly. A malformed window string fails loud at boot (`config.validate()`) rather than silently doing nothing. Configured windows are surfaced read-only via `alerts.thresholds_status()`. Covered by `test_parse_maintenance_windows_shapes`, `test_in_maintenance_window_boundaries_and_wraparound`, `test_config_validate_rejects_malformed_maintenance_window`, `test_maintenance_window_suppresses_down_alert`, `test_maintenance_window_does_not_suppress_other_backends`.
...
- **Four `aimon.litellm` DEBUG lines are now self-explanatory instead of opaque.** `/health/backlog -> N` didn't say what N counted → now `/health/backlog: in_flight_requests=N`. `/spend lite: … errs=(None,None,None)` didn't say which of the 3 calls each slot was → now labelled `errs=(activity=…, per_model=…, top_keys=…)`. `/team directory: teams=0 users_mapped=0 names=0` (then `names=39` a minute later with `teams`/`users_mapped` still 0) looked like one inconsistent source → now tagged `(via /team/list)` / `(via /user/list, independent paginated walk — can differ …)`, since they genuinely are two unrelated data sources. `daily spend via=none dates=0` gave no way to tell which spend-report variant(s) were tried or why they failed → `_daily_spend_by_date()` now returns a per-variant `attempts` trail (`tried=["report_capped:err=400", "report_30d:empty", …]`) logged alongside the terminal `via=`. No behavior change — log text only; `_daily_spend_by_date`'s internal return signature widened (2-tuple → 3-tuple) at its one call site.

### Fixed
- **A flapping backend no longer floods the webhook channel or the "Recent deliveries" list with `down:<name>` pairs.** The backend-down alert had asymmetric hysteresis: it required `ALERT_BACKEND_DOWN_AFTER` (3) consecutive failures to fire, but a **single** good poll cleared it — so a backend whose `/health` flaps under load (e.g. LiteLLM) emitted a recovery and then re-fired on the next failure, bypassing `ALERT_REPEAT_MIN` and spamming both Teams and the deliveries card. A DOWN backend is now latched and only declared back UP after `ALERT_BACKEND_UP_AFTER` (default 3) **consecutive** good polls; during that grace window the alert stays DOWN and keeps the real error text. Covered by `test_backend_down_recovery_needs_stable_up_no_flap`.
- **The /alerts "Recent deliveries" card collapses consecutive-identical runs.** A repeating alert (one flapping `down:litellm` re-notifying every cooldown) used to fill the whole 10-row list and crowd out distinct sends (a *Send test* click, a different alert). `recent_webhook_sends` now fetches a wider window and merges consecutive-identical `(channel, akey, ok, status)` runs into one row showing `×N` (newest time), so distinct recent deliveries stay visible. Covered by `test_delivery_card_collapses_consecutive_repeats`.
...
- **"Recent deliveries" list on the /alerts Channels card** — the last 10 webhook sends, newest first: when · which alert · outcome · round-trip latency (e.g. `3m ago · down:vllm · ✓ HTTP 204 · 41ms`). Delivery outcomes previously existed **only in the logs**, so the dashboard could never answer *"did that alert actually arrive?"* — the `alert_log`-backed "Alert history" card below records what was **evaluated** (fire/recover), not what was **delivered**. The list shows the **status code**, not just a tick, precisely because of the note above: a Teams endpoint that accepts-then-drops still reads `✓ HTTP 202` and stays diagnosable, while a timeout/refused connection reads `✗ no reply` with no status — so *rejected* and *never arrived* are visibly different. Backed by a new `webhook_log` table (pruned on the same 1-year horizon as `alert_log`); recording runs **off the event loop** and swallows its own errors, so this bookkeeping can never delay or break an actual alert. Covered by `test_webhook_log_records_and_reads_recent_deliveries`, `test_webhook_log_never_raises_on_bad_input`, `test_post_json_records_delivery_outcome`, `test_alerts_endpoint_exposes_recent_deliveries`, `test_alerts_channels_card_lists_recent_webhook_deliveries`.
