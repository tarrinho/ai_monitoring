...
### Fixed
- `/litellm`: hovering a chart now shows the full key/user/model name, not the truncated
  `first8…last4` string, on every chart that shortens labels (bar-chart tooltips fixed to
  read `chart.$full` instead of the truncated axis label; line/stacked-chart tooltips now
  show `dataset._k` instead of the truncated legend label; canvas-drawn legends get a new
  hover tooltip via `wireLegendFullName`, since they have no real DOM node for a native
  `title` to attach to). Centralized the 3 duplicate truncation functions into one
  `_shortLbl()` in `aimon-core.js`. `/spend`: the "keys used by X" detail list and the
  "N models could not be priced" note now carry a `title` with the untruncated text.
