...
| `key_series` (+`_1m`/`_1h`) | every tick / rollup | raw 24h / 30d / 1y | `/api/keyrequests` · `/api/keydelta` · `/api/litellm/concurrency-by-key` · `/api/keyseries` (the windowed, not all-time, "Top 10 API keys over time" card) |
